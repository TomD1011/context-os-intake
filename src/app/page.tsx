...
type StreamEvent =
  ...
  // Bot signalled completion but the summary is generated by /api/complete
  // (off the chat-stream budget). The client finalises by calling that endpoint.
  | { type: 'complete_pending'; session_id: string }
  | { type: 'done' }
...
export default function IntakePage() {
  ...
  const streamChat = useCallback(async (msgs: Message[]): Promise<void> => {
    ...
    let pendingCompleteSessionId: string | null = null
    ...
    while (true) {
      ...
      while ((newlineIdx = buffer.indexOf('\n')) !== -1) {
        ...
        } else if (event.type === 'complete_pending') {
          pendingCompleteSessionId = event.session_id
    ...
    // ─── Completion hand-off ────────────────────────────────────────────────
    // The bot signalled it's done, but the structured summary is generated by
    // the dedicated /api/complete endpoint (maxDuration 300) rather than inline
    // on the chat stream — that inline call is what used to time out and lose
    // the brain. This runs inside streamChat, so isStreaming stays true and the
    // user keeps seeing the busy state. /api/complete is idempotent, so a retry
    // after a flaky connection is safe.
    if (pendingCompleteSessionId) {
      setMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
          content:
            'Packaging your intake — this can take a minute. You can leave this tab open.',
        },
      ])
      const res = await fetch('/api/complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: pendingCompleteSessionId }),
      })
      if (!res.ok) {
        let errMsg = `Finalising failed (${res.status})`
        try {
          const j = await res.json()
          if (j?.error) errMsg = j.error
        } catch {
      ...
      const data = await res.json()
      if (data?.summary) setSummary(data.summary as IntakeSummary)
      else throw new Error('Completion returned no summary.')
    }
  }, [])
